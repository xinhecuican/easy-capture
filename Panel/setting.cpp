...
Setting::Setting(QWidget *parent) :
    WindowBase(parent, this, "Setting"),
    ui(new Ui::Setting) {
    ui->setupUi(this);
...
void Setting::normal_settings() {
    ...
    for(int i=Config::update_checktime_begin; i<=Config::update_checktime_end; i++) {
        update_name.push_back(Config::getConfigName(Config::setting(i)));
    }
    ...
    Config::update_checktime_begin, Config::update_checktime_end, [=](int index) {
        int before_index = normal_setting->getDefaultIndex("update_type");
        ready_setting.append(data(before_index, false));
        ready_setting.append(data(index+normal_setting->getBeginIndex("update_type"), true));
        data temp_data;
    ...
    for(int i=Config::languages_begin; i<=Config::languages_end; i++) {
        language_name.push_back(Config::getConfigName(Config::setting(i)));
    }
    ...
    Config::languages_begin, Config::languages_end, [=](int index) {
        int before_index = normal_setting->getDefaultIndex("language_type");
        ready_setting.append(data(before_index, false));
        ready_setting.append(data(index+normal_setting->getBeginIndex("language_type"), true));
        normal_setting->set_dirty(true);
    });
...
//        int before_index = capture_setting->getDefaultIndex("capture_type");
//        ready_setting.append(data(before_index, false));
//        ready_setting.append(data(index+capture_setting->getBeginIndex("capture_type"), true));
//        capture_setting->set_dirty(true);
//    });
...
void Setting::keySettings() {
    keySetting = new Tab_widget("快捷键", this);
    keySetting->add_spacer("{LvA0JggRsZ}全局快捷键");
//    QHBoxLayout* hlayout = new QHBoxLayout();
//    QLabel* label1 = new QLabel(MString::search("{iy3cIujuw5}打开截屏窗口"), keySetting);
//    QLabel* label2 = new QLabel("Ctrl+F1", keySetting);
//    QHBoxLayout* hlayout2 = new QHBoxLayout();
//    QLabel* label3 = new QLabel(MString::search("{0NHvAJ2fEQ}截取全屏"), keySetting);
//    QLabel* label4 = new QLabel("Ctrl+F2", keySetting);
//    hlayout->addWidget(label1);
//    hlayout->addWidget(label2);
//    hlayout2->addWidget(label3);
//    hlayout2->addWidget(label4);
//    keySetting->add_layout(hlayout);
//    keySetting->add_layout(hlayout2);
    QList<QString> globalKeyNames = KeyManager::getGlobalKeyName();
    for(int i=0; i<globalKeyNames.size(); i++) {
        keySetting->addGlobalKeyOption(i, "{" + globalKeyNames[i] + "}" + globalKeyNames[i], globalKeyNames[i]);
    }
    ...
    for(int i=0; i<window_name.size(); i++) {
        ...
        key_name.sort();
        keySetting->add_spacer("{" + window_name[i] + "}" + window_name[i]);
        int size = key_name.size();
        for(int k=0; k<size; k++) {
            keySetting->add_key_option(i * size + k, "{" + key_name[k] + "}" + key_name[k],
                                       window_name[i], key_name[k]);
        }
    }
    keySetting->done();
    all_setting.append(keySetting);
    ui->tabWidget->addTab(keySetting, MString::search("{YRHJ1nexv6}快捷键"));
}
